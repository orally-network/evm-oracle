 ...
 */
/// @custom:oz-upgrades-from OrallyVerifierOracle
contract OrallyVerifierOracleV2 is IOrallyVerifierOracle, OwnableUpgradeable {
     ...
     */
    function _verify(bytes memory _genericBytes, bytes memory _signature) internal view returns (bytes memory) {
        require(_verifyPacked(keccak256(_genericBytes), _signature), "InvalidSignature");
    ...
    function _verifyGeneric(bytes memory _data) internal view returns (bytes memory) {
        (bytes memory genericBytes, bytes memory signature) = abi.decode(_data, (bytes, bytes));

        return _verify(genericBytes, signature);
    }
